...
add_action('wp_ajax_nopriv_get_widget_data', 'get_widget_data');
add_action('wp_ajax_get_widget_data', 'get_widget_data');
...
{
    $path_to_config = WP_PLUGIN_DIR . "/" . GS_PLUGIN_DIR . '/widgets/' . $widget . '/config.php';
    ...
    //    file_put_contents(GS_API_CACHE_DIR."/$widget", $latestJSON); echo "line 74";
    return $latestJSON;
}
...
function buildRequestUrl($path_to_config, $widget)
{
    ...
    // check for limit
    if (defined('LIMIT') && LIMIT != '') { // if limit is defined keep it
        $limit = LIMIT;
    ...
    $api_key = get_option('gstc_apiKey');
