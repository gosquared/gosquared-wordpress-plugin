...
function write_apiproxy_config($config) {
    ...
    if (!$timeout || $timeout < MIN_TIMEOUT) $timeout = 5;
    $apiconfig = @fopen($config,"w");
    if (!$apiconfig) return 3;
    if (!flock($apiconfig, LOCK_EX)) return 4;
    if (!fwrite($apiconfig, 
        "<?php define('API_KEY','".$apiKey."'); define('SITE_TOKEN','".$acct."'); define('CACHE_TIMEOUT', ".$timeout."); ?>"))
        return 5;
...
<?php
	...
	if(isset($_POST['gs_acct'])){
		...
		if($valid_acct && $valid_apiKey){
			...
			$config_dir = WP_PLUGIN_DIR."/gosquared/apiproxyconfig.php";
	...
	<form name="gs-options" action="" method = "post">
        ...
        <table class="gs-settings">	    
	       <tr>
	           ...
	           <td><input type="radio" name="gs_trackAdmin" value="Yes" id="trackAdmin" <?php if($trackAdmin == 'Yes') echo 'checked="checked" '; ?>/> Yes</td>
	           <td><input type="radio" name="gs_trackAdmin" value="No" id="trackAdmin" <?php if($trackAdmin == 'No') echo 'checked="checked" '; ?>/> No</td>
	       </tr>
		   <tr>
		      <td class="label">Track post preview pages</td>
		      <td><input type="radio" name="gs_trackPreview" value="Yes" id="trackPreview" <?php if($trackPreview == 'Yes') echo 'checked="checked" '; ?>/> Yes</td>
		      <td><input type="radio" name="gs_trackPreview" value="No" id="trackPreview" <?php if($trackPreview == 'No') echo 'checked="checked" '; ?>/> No</td>
		   </tr>
		   <tr>
		      <td class="label">Tag individual users with </td>
		      <td><input type="radio" name="gs_trackUser" value="Off" id="trackUser" <?php if($trackUser == 'Off') echo 'checked="checked" '; ?>/> Off</td>
		      <td><input type="radio" name="gs_trackUser" value="UserID" id="trackUser" <?php if($trackUser == 'UserID') echo 'checked="checked" '; ?>/> User ID</td>
		      <td><input type="radio" name="gs_trackUser" value="Username" id="trackUser" <?php if($trackUser == 'Username') echo 'checked="checked" '; ?> /> Username</td>
		      <td class="wide"><input type="radio" name="gs_trackUser" value="DisplayName" id="trackUser" <?php if($trackUser == 'DisplayName') echo 'checked="checked" '; ?>/> Display Name</td>
		   </tr>
           <tr>
               <td class="label">Set widget cache timeout</td>
               <td><input type="radio" name="gs_cacheTimeout" value="5" id="chacheTimeout" <?php if($cacheTimeout == 5) echo 'checked="checked"'; ?>/>5s</td>
               <td><input type="radio" name="gs_cacheTimeout" value="15" id="chacheTimeout" <?php if($cacheTimeout ==
                   15) echo 'checked="checked"'; ?>/>15s</td>
               <td><input type="radio" name="gs_cacheTimeout" value="30" id="chacheTimeout" <?php if($cacheTimeout ==
                   30) echo 'checked="checked"'; ?>/>30s</td>
               <td><input type="radio" name="gs_cacheTimeout" value="60" id="chacheTimeout" <?php if($cacheTimeout ==
                   60) echo 'checked="checked"'; ?>/>60s</td>
           </tr>
	   </table>
...
function gs_print_gstc(){
    ...
    $gstc_userDetail = '';
		...
		wp_enqueue_script('gstc', WP_PLUGIN_URL .'/gosquared/tracker.js', '', false, true);
...
class WP_Widget_GS_OnlineVisitors extends WP_Widget {
    ...
    function form($instance) {
        ...
        <label for="gs-widget-option-1" class="gs-widget-label">
            <div class="gs-img-container"><img src="<? echo WP_PLUGIN_URL; ?>/gosquared/gs-widget-1.jpg" align="top" alt="style 1"/></div>
            <input type="radio" id="gs-widget-option-1" name="<?php echo $this->get_field_name('style'); ?>" 
                <?php if( 1 == $wstyle ) echo 'checked';?> value="1" />
        </label>

        <label for="gs-widget-option-2" class="gs-widget-label">
            <div class="gs-img-container"><img src="<? echo WP_PLUGIN_URL; ?>/gosquared/gs-widget-2.jpg" align="top" alt="style 2"/></div>
            <input type="radio" id="gs-widget-option-2" name="<?php echo $this->get_field_name('style'); ?>" 
                <?php if( 2 == $wstyle ) echo 'checked';?> value="2" />
        </label>

        <label for="gs-widget-option-3" class="gs-widget-label">
            <div class="gs-img-container"><img src="<? echo WP_PLUGIN_URL; ?>/gosquared/gs-widget-3.jpg" align="top" alt="style 3"/></div>
            <input type="radio" id="gs-widget-option-3" name="<?php echo $this->get_field_name('style'); ?>"
                <?php if( 3 == $wstyle ) echo 'checked';?> value="3" />
        </label>
    ...
    function widget($args, $instance) {
        ...
        ?> <script type="text/javascript">
                var wordpress_plugin_url = '<?php echo WP_PLUGIN_URL; ?>';
                var cache_timeout = <?php echo get_option('gstc_cacheTimeout'); ?> * 1000; // seconds -> milliseconds 
           </script><?php
        $widget_url = WP_PLUGIN_URL."/gosquared/widgets/gs-onlinevisitors/".$instance['style']."/gs-onlinevisitors.html";
